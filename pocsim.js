...
function failure() {
	...
	for (let node of failures) {
		if (node.fail_ends <= time) {
			node.online = true
			console.log(`${time}: [${node.addr}] is back online`);
		} else {
	...
	if (Math.random() < fail_chance) {
		...
		if (node.online) {
			...
			failures.push(node)
			console.log(`${time}: [${id}] went offline`);
		}
	}
}
...
function think(node, proposer) {
	...
	if (node === proposer) {
		...
		if (!node.block) {
			...
			node.started = time;
			node.proposed++;
			console.log(`${time}: [${node.addr}] proposing block at height ${block.height}`);
...
function receive(node, name, data, from) {
	if (!node.online) {
		console.log(`${time}: [${node.addr}] offline: missed <= [${from}] ${name}`);
		return;
	}
	node.received++;
	...
	switch (name) {
		case 'attest': {
			...
			if (!block.sig == ring[turn]) {
				console.log(`${time}: [${node.addr}] rejected attestation: not node ${block.sig}'s turn`);
				return;
			}
			if (time > block.time + attest_interval) {
				console.log(`${time}: [${node.addr}] rejected attestation: block is too old: ${block.time} vs ${time}`);
				return;
			}
			if (block.height !== node.height+1) {
				console.log(`${time}: [${node.addr}] rejected attestation: block has wrong height: ${block.height} expected ${node.height+1}`);
				return;
			}
			node.attested++;
		...
		case 'attested': {
			...
			if (block != null) {
				...
				console.log(`${time}: [${node.addr}] received attestation; have=${block.attest.length} start=${node.started} time=${time}`);
				...
				if (block.attest.length == 3 && time < node.started + attest_interval) {
					...
					console.log(`${time}: [${node.addr}] minted block at height ${block.height}`);
		...
		case 'block': {
			...
			if (block.hash != node.tip) {
				if (block.height !== node.height + 1) {
					console.log(`${time}: [${node.addr}] rejected block: wrong height: ${block.height} expected ${node.height+1}`);
				} else if (block.attest.length !== 3) {
					console.log(`${time}: [${node.addr}] rejected block: not enough attestations`);
				} else if (time > block.time + block_interval) {
					console.log(`${time}: [${node.addr}] rejected block: block is too old: ${block.time} vs ${time}`);
				} else {
					node.height = block.height
					node.tip = block.hash;
					node.accepted++;
					console.log(`${time}: [${node.addr}] accepted block at height ${block.height}`);
...
function tick() {
	...
	if (step == 0) {
		...
		num_intervals++;
		console.log(`${time}: start of turn ${ring[turn]} at block-interval ${num_intervals}`);
	}
